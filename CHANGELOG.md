...
- **[Windows/DirectX] MSAA support (`RenderPipelineDescriptor.sampleCount`, `ColorAttachment.resolveTarget`) was completely unimplemented** — the Metal/Vulkan work above never touched `src/directx/*`. `Device::createRenderPipeline()` hardcoded `SampleDesc.Count = 1` regardless of `sampleCount`, and no `ResolveSubresource` call existed anywhere in the backend, so a resolve target silently received no content at all. Fixed: `createRenderPipeline()` now honors `sampleCount`, and `RenderPassEncoder::end()` performs a real `ResolveSubresource`, correctly sequenced through the `RESOLVE_SOURCE`/`RESOLVE_DEST` states D3D12 requires — mirrors the Vulkan backend's resolve handling. Verified on real hardware (Intel Iris Graphics 550): a resolve-only clear and a real MSAA draw both now produce the exact expected resolved pixel content, in both Release and D3D12-debug-layer-enabled Debug builds.
- **[Windows/DirectX] `Texture::download()` hardcoded its transition barrier's before/after state to `D3D12_RESOURCE_STATE_COMMON`**, ignoring `TextureHandle::currentState` entirely — wrong for any texture just used as a render target (left in a shader-readable state, not `COMMON`), and left the tracked state stale afterward so the *next* pass reusing that texture could barrier from the wrong assumed state (surfaced as a D3D12 debug-layer resource-barrier mismatch). Pre-existing, general bug unrelated to MSAA — just never previously exercised by a texture that's downloaded after rendering and then reused in a later pass. Fixed by reading/restoring the real `currentState` instead of hardcoding `COMMON`. Verified on real hardware: the exact repro that surfaced this (clear → download → draw → download → clear → download, same textures reused throughout) now runs clean with zero debug-layer warnings and exactly correct pixel values at every step.
